  ...
  Lib.prototype = {
    ...
    default_specials: function () {
      ...
      if (this.options.twitter) {
        this.specials.define('twitter', /\[(https{0,1}:\/\/twitter\.com\/\w+\/status\/\d+)\]/ig, function (url) {
          return '<twitter>' + url + '</twitter>';
        });
      }

      if (this.options.slideshow) {
        this.specials.define('slideshow', /\[(https{0,1}:\/\/www\.slideshare\.net\/\w+\/.*?)\]/ig, function (url) {
          return '<slideshow>' + url + '</slideshow>';
        });
      }
      if (this.options.video) {
        ...
        var domain_pattern = 'https{0,1}:\/\/(www\.){0,1}';

        var regex = new RegExp('\\[(' + domain_pattern + site_patterns + ')\\]', 'ig');

        this.specials.define('video', regex, function (url) {
