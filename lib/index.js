...
module.exports = (function () {
  ...
  var version = '0.0.5',
    ...
    lib = {
      ...
      parse: function (contents, new_options) {
        ...
        contents = this.replaces.run(contents, false);
  ...
  lib.replaces = {
    registry: {},
    define: function (name, regex, callback, before, condition) {
      before = before || true;
      this.registry[name] = {
        regex: regex,
      ...
      for (name in this.registry) {
        if (this.registry.hasOwnProperty(name) && this.registry[name].before === before) {
          ...
          if (typeof data.condition === 'function') {
            condition = data.condition(options);
  ...
  lib.russian_quotes = function (string) {
    if (options.punctuation) {
      string = string.replace(/“([а-я!?.]+.*?[а-я!?.]+)”/ig, '«$1»');
    }
    return string;
  };

  //TODO: Make this (ALL THIS) a CLASS

  lib.replaces.define('russian_quotes', /“([а-я!?.]+.*?[а-я!?.]+)”/ig, '«$1»', false, function (options) {
    return options.punctuation;
  });
  lib.replaces.define('yo_capital', /Ё/g, 'Е', false, function (options) {
    return options.remove_yo;
  });
  lib.replaces.define('yo_small', /ё/g, 'е', false, function (options) {
    return options.remove_yo;
  });
